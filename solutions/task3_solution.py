...
# тут ваше решение:
def randomness_calculation(n, temp_list):
    randomness = 0
    if n == 1:
        return 1
    ...
    for i in range(n):
        if i == 0 and temp_list[i] > temp_list[i + 1]:
            randomness += 1
        if i == n - 1 and (temp_list[i] > temp_list[i - 1]):
            randomness += 1
        if i != 0 and i != n - 1 and temp_list[i] > temp_list[i - 1] and \
...
for input in inputs:
    ...
    print(randomness_calculation(n, temp_list))
